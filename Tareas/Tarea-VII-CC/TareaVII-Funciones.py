...
def posterior_log(alfa, beta, n, logr_1, r_2,c=1):
    ...
    Parameters
    ----------
    alfa :  Parámetro de forma de la distribución Gamma 
    beta :         Parámetro de tasa de la distribución Gamma
    n :         Tamaño de la muestra
    logr_1 :        logaritmo del  producto de las observaciones 
    r_2 :         Suma de las observaciones 
    c:          Parametro de la exponencial a priori
    Returns
    -------
...
def metropolis_hastings(n, logr_1, r_2, num_samples=15000, sigma_1=1.2, sigma_2=5, seed=123,c=1):
    ...
    sigma_1 :         Desviación estándar para propuestas de alpha
    sigma_2 :         Desviación estándar para propuestas de beta
    seed :         Semilla para reproducibilidad
        c: Parametro de la exponencial(Apriori para beta)
    Returns
    -------
    ...
    beta_init = np.random.exponential(1/c)  # Exponencial(1)
    ...
    for i in range(num_samples):
        ...
        log_post_current = posterior_log(alpha_c, beta_c, n, logr_1, r_2)
        log_post_proposed = posterior_log(alpha_p, beta_p, n, logr_1, r_2)
...
def graficar_diagnosticos_mcmc(samples, samples_burnt, a, b, burn_in,n):
    ...
    # =========================================================================
    # SEGUNDA FILA: DISTRIBUCIONES MARGINALES
    # =========================================================================
    
    # Histograma de la distribución marginal de alpha
    ...
    # Histograma de la distribución marginal de beta
...
def graficar_espacio_parametros(samples, samples_burnt, a, b, n, logr_1, r_2, mostrar_trayectoria=True):
    """
    ...
    logr_1 : logaritmo del roducto de las observaciones
    r_2 : float
    ...
    beta_grid = np.linspace(0.1, 40, 50)
    ...
    for i, alpha_val in enumerate(alpha_grid):
        for j, beta_val in enumerate(beta_grid):
            log_post_grid[i, j] = posterior_log(alpha_val, beta_val, n, logr_1, r_2)
    ...
    plt.clabel(contour_main, inline=True, fontsize=9)
...
def ejecutar_analisis_completo(n_muestras, a_verdadero=3, b_verdadero=100, seed=123):
    """
    Función principal que ejecuta el análisis completo: simulación de datos,
    MCMC y visualización de resultados.
    
    Parameters
    ----------
    n_muestras : int
        Tamaño de muestra (5 o 40 para este ejercicio)
    a_verdadero : float
        Valor verdadero del parámetro alpha
    b_verdadero : float
        Valor verdadero del parámetro beta
    seed : int
        Semilla para reproducibilidad
    """
    print(f"=== ANÁLISIS PARA n = {n_muestras} ===")
...
# Para calcular HPDI 
    hdi_alpha = az.hdi(samples_burnt[:, 0], hdi_prob=0.95)
    hdi_beta = az.hdi(samples_burnt[:, 1], hdi_prob=0.95)
    print(f"HPDI 95% α: [{hdi_alpha[0]:.2f}, {hdi_alpha[1]:.2f}]")
    print(f"HPDI 95% β: [{hdi_beta[0]:.2f}, {hdi_beta[1]:.2f}]")
    # 6. Generar gráficos
    graficar_diagnosticos_mcmc(samples, samples_burnt, a_verdadero, b_verdadero, burn_in,n_muestras)

# Segundo: gráficos del espacio de parámetros
    graficar_espacio_parametros(samples, samples_burnt, a_verdadero, b_verdadero, n_muestras, logr_1, r_2, mostrar_trayectoria=True)
    return samples_burnt
...
if __name__ == "__main__":
    ...
    for n in casos:
        ...
        resultados_n = ejecutar_analisis_completo(n_muestras=n)
    
    # Extraer muestras de alpha y beta después del burn-in
        alpha_samples = resultados_n[:, 0]  # Columna 0: alpha
        beta_samples = resultados_n[:, 1]   # Columna 1: beta
    
    # Graficar autocorrelación para alpha y beta
    ...
    # Aplicar thinning (submuestreo cada 50 iteraciones)
        alpha_thinned = alpha_samples[::50]
        beta_thinned = beta_samples[::50]
    
    # Autocorrelación después de thinning
        plot_acf(alpha_thinned, lags=50, ax=axes[0, 1], title="Autocorrelación α (thinning=50)")
        plot_acf(beta_thinned, lags=50, ax=axes[1, 1], title="Autocorrelación β (thinning=50)")
    ...
    # Mostrar tamaños de muestra
        ...
        print(f"Muestras después de thinning (cada 50): {len(alpha_thinned)}")
