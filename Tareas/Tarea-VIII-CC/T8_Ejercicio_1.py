...
def Distribucion_Objetivo(mu, sigma1, sigma2, rho):
    ...
    -rho relación lineal de la covarianza.
    """
...
def graficar_espacio_parametros(samples, samples_burnt, mu_verdadero, rho_verdadero, titulo=""):
    ...
    contour_main = plt.contour(X1, X2, Z, levels=20, colors='black', 
...
if __name__ == "__main__":
    ...
    print("=== METROPOLIS-HASTINGS CON KERNEL HÍBRIDO (3 KERNELS) ===\n")
    
    for rho in Rho:
        print(f"SIMULACIÓN PARA ρ = {rho}")
        ...
        print(f"Tasas de aceptación por kernel:")
        print(f"  Kernel 0 (rechazo): {tasas_acept[0]:.3f}")
        print(f"  Kernel 1 (X1|X2): {tasas_acept[1]:.3f}")
        print(f"  Kernel 2 (X2|X1): {tasas_acept[2]:.3f}")
